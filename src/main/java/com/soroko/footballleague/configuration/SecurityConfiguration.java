...
import com.soroko.footballleague.service.JwtAuthenticationFilter;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
       /* return http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/account/registration", "/account/login", "/", "/teams")// запросы
                        .permitAll()// разрешены всем
                        .dispatcherTypeMatchers(FORWARD, ERROR).permitAll()// для отображения html
                        .requestMatchers("/task", "/task/**") // запросы
                        .hasAnyRole("USER", "MODERATOR", "ADMIN") // разрешены только пользователям с указанными ролями
